...
% Displays maximum latereal force for every Frontal Area option
...
US_Mass_r = 46;% Kg
Wheelbase = 2.890;% m
Weight_distribution = [0.45, 0.46, 0.47, 0.48, 0.49, 0.5, 0.51, 0.52, 0.53, 0.54, 0.55]; %
track_f = 1.698;% m
track_r = 1.620;% m
hcg = 0.45;% m
hrf = 0;% m
...
number_of_iterations = length(Weight_distribution) * length(K_spring_F) * length(K_spring_R)...
    * length(K_ARB_F) * length(K_ARB_R) * length(Area);
z=0; % General pointer
...
Fz_downforce = zeros(2,number_of_iterations); % Empty array of zeros for output Fz Downforce
            ...
            for l=1:numel(K_ARB_F)
                ...
                for m=1:numel(K_ARB_R)
                    ...
                    for n=1:numel(Area)
                        ...
                        Fz_downforce(1,z) = Fz_downforce_i; % Current run execution's Downforce
                        Fz_downforce(2,z) = area; % Current run execution's Frontal Area
